...
- Hooks 接口存在，但当前 DareAgent 未实际触发 hooks（TODO）。

## 4. 关键接口与实现

- Kernel：`IAgent`（`dare_framework/agent/kernel.py`）
- 编排策略接口：`IAgentOrchestration`（`dare_framework/agent/interfaces.py`）
- 默认实现：`DareAgent`（`dare_framework/agent/_internal/five_layer.py`）
- 轻量实现：`SimpleChatAgent`（`dare_framework/agent/_internal/simple_chat.py`）
- Builder：`DareAgentBuilder` / `SimpleChatAgentBuilder`（`dare_framework/agent/_internal/builder.py`）
...
- TODO: Hooks 生命周期触发点与默认 hook 管理器。

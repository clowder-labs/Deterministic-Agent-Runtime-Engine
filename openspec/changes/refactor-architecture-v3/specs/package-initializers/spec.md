...
- **THEN** the protocol is defined in `dare_framework3_3/tool/kernel.py`, not in `__init__.py`.

#### Scenario: Finding a model definition
- **WHEN** a contributor needs to inspect `RunResult`
- **THEN** it is defined in `dare_framework3_3/plan/types.py`, not in `__init__.py`.

...
- **THEN** the protocol is defined in `dare_framework3/tool/component.py`, not in `__init__.py`.

#### Scenario: Finding a model definition
- **WHEN** a contributor needs to inspect `RunResult`
- **THEN** it is defined in `dare_framework3/plan/types.py`, not in `__init__.py`.

### Requirement: Package initializers are minimal except domain facades
Domain package `__init__.py` files MAY re-export public symbols; other package initializers (including `impl/`) SHALL contain only documentation, metadata, or re-export statements and SHALL NOT define classes or functions.

#### Scenario: Auditing an implementation package initializer
- **WHEN** auditing `dare_framework3/tool/impl/__init__.py`
- **THEN** the file contains only re-exports and no class or function definitions.

#### Scenario: Auditing a domain initializer
- **WHEN** auditing `dare_framework3/tool/__init__.py`
...
The framework SHALL avoid modules whose sole purpose is re-exporting symbols, except for domain `__init__.py` files and `impl/__init__.py` re-export lists.

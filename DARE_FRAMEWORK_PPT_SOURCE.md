...
| 层级 | 组件 | 源码路径 | 扩展方式 |
|------|------|----------|----------|
| **L3 Builder** | DareAgentBuilder | agent/builder.py | 继承 _BaseAgentBuilder，实现 _build_impl() |
| **L3 Builder** | ReactAgentBuilder | agent/builder.py | 同上 |
| **L3 Builder** | ChatAgentBuilder | agent/builder.py | 同上 |
| **L2 编排** | DareAgent | agent/five_layer.py | 实现 IAgentOrchestration，或新增编排策略 |
| **L2 编排** | ReactAgent | agent/react_agent.py | 同上 |
| **L2 编排** | ChatAgent | agent/simple_chat.py | 同上 |
| **L1 Context** | Context | context/_internal/context.py | 实现 IContext，可覆写 assemble() |
| **L1 Model** | OpenRouterModelAdapter | model/openrouter_adapter.py | 实现 IModelAdapter.generate() |
...
| **L1 Memory** | InMemorySTM | memory/in_memory_stm.py | 实现 IShortTermMemory（add/get/clear/compress） |
...
**内置实现**（位于 `dare_framework/model/*.py`）：
...
- **IContext**：id, short_term_memory, budget, long_term_memory, knowledge, config；stm_add/stm_get/stm_clear；budget_use/budget_check；listing_tools；assemble；compress
...
assemble(**options)
    ...
    │           IF callable: _tool_list = list_tool_defs()
    │           ELSE: _tool_list = provider.list_tools() 转 dict 兼容
    │       return _tool_list or []
    │

...
以 HTTP 方式对外提供 MCP 协议，需**先单独启动**本进程，再启动 Agent 连接。

用法：
  终端 1：python local_mcp_server.py          # 服务默认监听 http://127.0.0.1:8765
  终端 2：cd examples/09-plan-agent-with-subagents && python main.py   # Agent 连接该服务
"""
...
def _parse_two_numbers(arguments: dict[str, Any]) -> tuple[float, float]:
    ...
    missing = [key for key in ("a", "b") if key not in arguments]
    if missing:
    ...
    try:
        ...
        raise ValueError("arguments 'a' and 'b' must be numbers") from exc

    if not math.isfinite(a) or not math.isfinite(b):
        raise ValueError("arguments 'a' and 'b' must be finite numbers")
...
def main() -> None:
    ...
    print("Start the Agent in another terminal; config should use url: http://127.0.0.1:8765/")

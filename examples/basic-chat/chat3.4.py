"""Basic chat example using dare_framework3_4.

This demonstrates the v3.4 context-centric architecture with:
- Context as the core entity (messages assembled on-demand)
- SimpleChatAgent for conversational interactions
- Short-term memory managed by Context
"""
...
from dare_framework3_4.agent import SimpleChatAgent
from dare_framework3_4.model import OpenAIModelAdapter
...
logger = logging.getLogger("basic-chat-v3_4")
...
async def main() -> None:
    ...
    logger.info(
        "boot basic chat agent (dare_framework3_4)",
        extra={
            "model": MODEL,
            "endpoint": ENDPOINT,
    ...
    # Create agent using SimpleChatAgent
    agent = SimpleChatAgent(
        name="basic-chat-v3_4",
        model=model_adapter,
    )
    ...
    print("SimpleChatAgent v3.4 - Type your message (or /quit to exit):", flush=True)
    while True:
        try:
            raw = input("You: ")
        except EOFError:

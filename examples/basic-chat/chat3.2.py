"""Basic chat example using dare_framework3_2.

This demonstrates the v3.2 architecture with:
- event/ and hook/ as separate domains
- No factory functions, direct import of implementation classes
- interfaces.py renamed to component.py
- runtime/ deleted, responsibilities distributed
"""
...
# Clean imports from dare_framework3.2
# Note: In v3.2, we import directly from the framework
from dare_framework3_2 import FiveLayerAgent

# Model adapter - direct import from impl
from dare_framework3_2.model import OpenAIModelAdapter

# Tools - direct import from impl
from dare_framework3_2.tool import RunCommandTool
...
logger = logging.getLogger("basic-chat-v3_2")
...
async def main() -> None:
    ...
    logger.info(
        "boot basic chat agent (dare_framework3_2)",
        extra={
            "model": MODEL,
            "endpoint": ENDPOINT,
    ...
    # Create agent using dare_framework3.2's FiveLayerAgent
    # v3.2 architecture: Agent directly organizes components (dare_framework3_2)
    agent = FiveLayerAgent(
        name="basic-chat-v3_2",
        model=model_adapter,

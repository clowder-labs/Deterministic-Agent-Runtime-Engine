...
class AgentControlHandler:
    ...
    async def invoke(self, control: AgentControl, params: dict[str, Any] | None = None) -> Any:
        if control == AgentControl.INTERRUPT:
            result = self._agent.interrupt() if params is None else self._agent.interrupt(**params)
        elif control == AgentControl.PAUSE:
            result = self._agent.pause() if params is None else self._agent.pause(**params)
        elif control == AgentControl.RETRY:
            result = self._agent.retry() if params is None else self._agent.retry(**params)
        elif control == AgentControl.REVERSE:
            result = self._agent.reverse() if params is None else self._agent.reverse(**params)

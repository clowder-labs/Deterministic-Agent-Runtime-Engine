...
>
> 最终架构设计以 `docs/design/Architecture.md` 为准；本文档仅描述“当前实现视角”，并尽量通过引用保持与权威设计一致。
>
...
- `dare_framework/core/`：当前实现中的 Kernel contracts + 数据结构（设计基线：`docs/design/Architecture.md`；接口清单：`docs/design/Interfaces.md`）
...
3. 编排驱动五层循环骨架（Session/Milestone/Plan/Execute/Tool）（当前实现：`IRunLoop` → `ILoopOrchestrator`；设计文档：编排归入 `agent` 域，详见 `docs/design/Architecture.md`）
...
2. `docs/design/Architecture.md`（最终架构设计：权威）
3. `docs/design/Interfaces.md`（权威接口清单）
4. `docs/design/DARE_alignment.md`（对齐清单：claims → 证据/实现）
5. `docs/guides/Development_Constraints.md`（开发约束清单）
6. `docs/design/archive/Architecture_Final_Review_v2.1.md` / `docs/design/archive/Architecture_Final_Review_v1.3.md`（历史参考）

"""Default loop orchestrator implementation."""
...
from dare_framework2.execution.types import ResourceType, HookPhase
...
class DefaultLoopOrchestrator(ILoopOrchestrator):
    """Default five-layer loop orchestrator.
    
    Implements the Session -> Milestone -> Plan -> Execute -> Tool loop.
    """
    ...
    async def run_plan_loop(self, milestone: Milestone) -> ValidatedPlan:
        ...
        if self._ext is not None:
            await self._ext.emit(HookPhase.BEFORE_PLAN, {
                "milestone_id": milestone.milestone_id,
                "run_id": self._run_id,
            })
        ...
        if self._ext is not None:
            await self._ext.emit(HookPhase.AFTER_PLAN, {
                "milestone_id": milestone.milestone_id,
                "run_id": self._run_id,
            })
        return validated
    ...
    async def run_tool_loop(self, req: ToolLoopRequest) -> ToolLoopResult:
        ...
        for attempt in range(int(max_calls)):
            ...
            if self._ext is not None:
                await self._ext.emit(HookPhase.BEFORE_TOOL, {
            ...
            if self._ext is not None:
                await self._ext.emit(HookPhase.AFTER_TOOL, {
                    "capability_id": capability_id,
                    "run_id": self._run_id,
                    "attempt": attempt,
                    "success": result.success,
